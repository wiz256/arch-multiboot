...
1. **Boot from Arch Linux or EndeavourOS Live ISO**
...
# Or use nmtui for graphical interface
nmtuiw/arch-multi
```
...
```bash
# One-liner install
curl -L https://raw.githubusercontent.com/wiz256/arch-multiboot/main/install-arch-multiboot.sh -o install.sh && chmod +x install.sh && sudo ./install.sh

# Or clone from git
git clone https://github.com/yourusername/arch-multiboot
cd arch-multiboot
sudo ./install-arch-multiboot.sh
```

## Usage:

```bash
# From Arch or EndeavourOS Live USB:
curl -O https://raw.githubusercontent.com/wiz256/arch-multiboot/main/install-arch-multiboot.sh
chmod +x install-arch-multiboot.sh
sudo ./install-arch-multiboot.sh
```

The script will now:
- Install Niri (not Hyprland)
- Set up complete Catppuccin theming across all toolkits
- Configure HiDPI console fonts for comfortable TTY use
- Mount shared-data for cross-Linux sharing
- Configure proper scaling at all levels (boot, TTY, desktop)


## Corrected Arch Multi-Boot Installation Script with Niri & Complete Configuration

### `install-arch-multiboot.sh` - Fixed Version with Niri, Proper Theming & TTY Scaling
...
✅ **Niri WM** instead of Hyprland - proper scrollable tiling compositor
✅ **Complete Catppuccin theming** for:
- GTK2, GTK3, GTK4
- Qt5 and Qt6 via qt5ct/qt6ct
- Kvantum theme for better Qt integration
- Cursors, icons, and all UI elements

✅ **Shared-data subvolume** properly mounted at `/mnt/shared-data`
✅ **HiDPI TTY font** - Terminus 24pt bold for comfortable reading
✅ **Early KMS** for better boot resolution
✅ **GRUB font** for HiDPI boot menu
✅ **Proper environment variables** for scaling and theming
✅ **Touch gestures** configured with libinput-gestures
✅ **Waybar** properly configured for Niri (not Hyprland)

## Usage:

```bash
# From Arch or EndeavourOS Live USB:
curl -O https://raw.githubusercontent.com/yourusername/arch-multiboot/main/install-arch-multiboot.sh
chmod +x install-arch-multiboot.sh
sudo ./install-arch-multiboot.sh
```
